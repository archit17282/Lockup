...
public class passrecover extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        confirmButton = (Button) findViewById(R.id.submitres2);
        ...
        confirmButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (questionNumber != 0 && !answer.getText().toString().isEmpty()) {
                    if (sharedPreferences.getInt(Constants.QUESTION_NUMBER, 0) == questionNumber && sharedPreferences.getString(Constants.ANSWER, "").matches(answer.getText().toString())) {
                        editor.putBoolean(Constants.IS_PASSWORD_SET, false);
                        editor.commit();
                        editor.putString(Constants.PASSWORD, "");
                        editor.commit();
                        Intent i = new Intent(passrecover.this, password.class);
                        startActivity(i);
                        finish();
                    } else {
